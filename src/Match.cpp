  ...
  if (matched) {
    ...
    do {
      ...
      for (auto c1 = constituents.begin(), c2 = constituents.begin() + 1; c2 != constituents.end(); c1++, c2++) {
        ...
        if (!anchored_deduce(first_constituent, second_constituent)) matched = false;
        number_of_unmatched++;
      }
    ...
    case Constituent::Type::CHARACTER:
      switch (c2.type) {
        ...
        case Constituent::Type::END:
          c2.possible_indices = { string.size(), string.size() };
          c2.matched          = true;
...
void Match::deduce_string_on_left(Constituent& s, Constituent& c2)
{
  ...
  if (start == std::string::npos) return;
    ...
    s.deduced_value    = string.substr(index, start2 - index);
    s.matched          = true;
